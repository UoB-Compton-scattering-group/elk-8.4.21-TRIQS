...
! spin-polarised
      if (ncmag) then
! non-collinear
        call rmk1(npc,wo,wfmt(:,1,i),wfmt(:,2,i),rhomt(:,ias),magmt(:,ias,1), &
         magmt(:,ias,2),magmt(:,ias,3))
      else
! collinear
        call rmk2(npc,wo,wfmt(:,1,i),wfmt(:,2,i),rhomt(:,ias),magmt(:,ias,1))
      end if
    else
! spin-unpolarised
      call rmk3(npc,wo,wfmt(:,1,i),rhomt(:,ias))
    end if
...
!$OMP PARALLEL DO DEFAULT(SHARED) &
!$OMP PRIVATE(wfir,wfgp,wo,i) &
!$OMP PRIVATE(ispn,jspn,n,ist,z1) &
!$OMP NUM_THREADS(nthd)
do j=1,nstsv
  wo=occsvp(j)
...
! generate spinor wavefunction from second-variational eigenvectors
    ...
    do ispn=1,nspinor
      ...
      do ist=1,nstfv
        ...
        call zaxpy(n,z1,evecfv(:,ist,jspn),1,wfgp,1)
      end do
      wfir(:,ispn)=0.d0
...
! spin-unpolarised wavefunction
    wfir(:,1)=0.d0
    wfir(igfc(igpig(1:ngp(1),1)),1)=evecfv(1:ngp(1),j,1)
