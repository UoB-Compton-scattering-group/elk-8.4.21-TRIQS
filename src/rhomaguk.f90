...
! generate the wavefunctions (single-precision in the muffin-tin)
...
! muffin-tin part
      do ias=1,natmtot
        ...
        if (spinpol) then
          if (ncmag) then
            call rmk1_sp(npc,wo,wfmt(:,ias,1,jst),wfmt(:,ias,2,jst), &
             rhormt(:,ias,ir),magrmt(:,ias,1,ir),magrmt(:,ias,2,ir), &
             magrmt(:,ias,3,ir))
          else
            call rmk2_sp(npc,wo,wfmt(:,ias,1,jst),wfmt(:,ias,2,jst), &
             rhormt(:,ias,ir),magrmt(:,ias,1,ir))
          end if
        else
          call rmk3_sp(npc,wo,wfmt(:,ias,1,jst),rhormt(:,ias,ir))
        end if
      end do
! interstitial part
      if (spinpol) then
...
pure subroutine rmk1_sp(n,wo,wf1,wf2,rho,mag1,mag2,mag3)
implicit none
! arguments
integer, intent(in) :: n
real(8), intent(in) :: wo
...
do i=1,n
  ...
  mag1(i)=mag1(i)+wo2*(a1*a2+b1*b2)
  mag2(i)=mag2(i)+wo2*(a1*b2-b1*a2)
  mag3(i)=mag3(i)+wos*(t1-t2)
  rho(i)=rho(i)+wos*(t1+t2)
end do
end subroutine
...
pure subroutine rmk2_sp(n,wo,wf1,wf2,rho,mag)
implicit none
! arguments
integer, intent(in) :: n
real(8), intent(in) :: wo
...
pure subroutine rmk3_sp(n,wo,wf,rho)
implicit none
! arguments
integer, intent(in) :: n
real(8), intent(in) :: wo
...
real(4) wos
wos=wo
rho(:)=rho(:)+wos*(real(wf(:))**2+aimag(wf(:))**2)
end subroutine
