...
! automatic arrays
complex(4) wfmt2(npcmtmax,nspinor)
complex(8) z(ngp)
! allocatable arrays
...
! apply local potential and magnetic field to spinor wavefunction
    if (ncmag) then
! non-collinear case
      call zvbmk1_sp(npc,zvmt(:,ias),zbmt(:,ias,1),zbmt(:,ias,2),zbmt(:,ias,3),&
       wfmt(:,ias,1,jst),wfmt(:,ias,2,jst),wfmt2,wfmt2(:,2))
    else
! collinear case
      call zvbmk2_sp(npc,zvmt(:,ias),zbmt(:,ias,1),wfmt(:,ias,1,jst), &
       wfmt(:,ias,2,jst),wfmt2,wfmt2(:,2))
    end if
! compute the inner products
    do ist=1,nstsv
      vbmat(ist,jst)=vbmat(ist,jst) &
       +cdotc(npc,wfmt(:,ias,1,ist),1,wfmt2,1) &
       +cdotc(npc,wfmt(:,ias,2,ist),1,wfmt2(:,2),1)
    end do
  end do
end do
!$OMP END PARALLEL DO
...
pure subroutine zvbmk1_sp(n,zv,zb1,zb2,zb3,wf11,wf12,wf21,wf22)
implicit none
! arguments
integer, intent(in) :: n
complex(8), intent(in) :: zv(n),zb1(n),zb2(n),zb3(n)
...
pure subroutine zvbmk2_sp(n,zv,zb,wf11,wf12,wf21,wf22)
implicit none
! arguments
integer, intent(in) :: n
complex(8), intent(in) :: zv(n),zb(n)
