...
integer lmmax,ia,ja,nthd,is,ias,jas,mst,isym
complex(8), allocatable :: z1(:,:), z2(:,:), z3(:,:)
complex(8) chg,z

!loop over each correlated orbital
do iorb=1,norb
  is=orb(iorb,1)
  l=orb(iorb,2)
  lmmax=2*l+1
...
!calculate the local Green's function for frequency w
          if(gf) then
            call gwank(lmmax,mst,ik,idxwan(:,ik),w,z1,z2,z3)
          else
...
! rotate dmat into equivalent atom
    do isym=1,nsymcrys
      ...
      if (.not.done(ja)) then
        jas=idxas(ja,is)
        dmat(i:j,ispn,i:j,jspn,jas)=dmat(i:j,ispn,i:j,jspn,ias)
        done(ja)=.true.
      end if
    end do
  end do
